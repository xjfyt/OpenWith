param(
	[string]$VSCodeExe = '',
	[string]$Title = "$([char]0x901A)$([char]0x8FC7) Code $([char]0x6253)$([char]0x5F00)",
	[switch]$SkipCompile
)
...
$SettingsKey = 'HKCU:\Software\Classes\OpenWithVSCodeContextMenu'

$ScriptDir = Split-Path -Parent $MyInvocation.MyCommand.Path
$WorkDir = Join-Path $ScriptDir 'win11-context-menu'
$RuntimeRoot = Join-Path $env:LOCALAPPDATA 'OpenWith\VSCodeContextMenu'
$SourcePath = Join-Path $WorkDir 'src\VSCodeExplorerCommand.cpp'
...
$DllPath = Join-Path $ExternalDir 'VSCodeExplorerCommand.dll'
$ObjPath = Join-Path $ExternalDir 'VSCodeExplorerCommand.obj'
$ManifestPath = Join-Path $ManifestDir 'AppxManifest.xml'
$AppxPath = Join-Path $PackageDir 'VSCodeContextMenu.appx'
$CertPath = Join-Path $PackageDir 'OpenWithVSCodeContextMenu.cer'
...
function Resolve-VSCodeExe([string]$ExplicitPath) {
	...
	foreach ($root in $uninstallRoots) {
		...
		foreach ($key in Get-ChildItem $root -ErrorAction SilentlyContinue) {
			...
			if (($item.DisplayName -like '*Visual Studio Code*') -or ($item.DisplayName -like '*VSCode*') -or ($item.InstallLocation -like '*Microsoft VS Code*') -or ($item.DisplayIcon -like '*Code.exe*')) {
				$possiblePaths = @()
				if ($item.InstallLocation) {
					$possiblePaths += Join-Path $item.InstallLocation 'Code.exe'
				}
				if ($item.DisplayIcon) {
	...
	throw 'Code.exe was not found. Re-run with -VSCodeExe "C:\Path\To\Code.exe".'
}
...
<Package
  ...
  <Properties>
    <DisplayName>VSCode Context Menu</DisplayName>
  ...
  <Applications>
    <Application Id="VSCodeContextMenu"
      Executable="Code.exe"
      ...
      <Extensions>
        ...
        <com:Extension Category="windows.comServer">
          <com:ComServer>
            <com:SurrogateServer DisplayName="VSCode Context Menu">
              <com:Class Id="$ClassId" Path="VSCodeExplorerCommand.dll" ThreadingModel="STA" />
            </com:SurrogateServer>
          </com:ComServer>
        </com:Extension>
      </Extensions>
...
function Set-ContextMenuSettings {
	...
	New-ItemProperty -Path $SettingsKey -Name 'ExePath' -Value $VSCodeExe -PropertyType ExpandString -Force | Out-Null
}
...
function Remove-LegacyRegistryVerbs {
	$keys = @(
		...
		'HKLM:\Software\Classes\*\shell\OpenWithVSCode',
		'HKLM:\Software\Classes\Directory\shell\OpenWithVSCode',
		'HKLM:\Software\Classes\Directory\Background\shell\OpenWithVSCode'
	)
...
function Ensure-SigningCertificate {
	...
	if (-not $cert) {
		$cert = New-SelfSignedCertificate `
			-Type Custom `
			-Subject $Publisher `
			-FriendlyName 'OpenWith VSCode Context Menu' `
...
$VSCodeExe = Resolve-VSCodeExe $VSCodeExe
Assert-FileExists $VSCodeExe 'Code.exe'
Compile-Dll
