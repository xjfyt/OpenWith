...
// {E07AE7AA-313B-418B-AB40-6FD29ABE24F1}
static const CLSID CLSID_AntigravityExplorerCommand =
{ 0xe07ae7aa, 0x313b, 0x418b, { 0xab, 0x40, 0x6f, 0xd2, 0x9a, 0xbe, 0x24, 0xf1 } };

static volatile LONG g_dllRefCount = 0;
static const wchar_t kSettingsKey[] = L"Software\\Classes\\AntigravityContextMenu";
static const wchar_t kFallbackTitle[] = L"Open with Antigravity";
...
public:
	...
	IFACEMETHODIMP Invoke(IShellItemArray* items, IBindCtx*) override {
		...
		for (DWORD i = 0; i < count; ++i) {
			...
			std::wstring args = QuoteForCommandLineArg(path);
			CoTaskMemFree(path);

			HINSTANCE result = ShellExecuteW(nullptr, L"open", exePath.c_str(), args.c_str(), nullptr, SW_SHOWNORMAL);
...
STDAPI DllGetClassObject(REFCLSID clsid, REFIID riid, void** object) {
	if (!IsEqualCLSID(clsid, CLSID_AntigravityExplorerCommand)) {
