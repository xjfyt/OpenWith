param(
	...
	[string]$Title = "$([char]0x901A)$([char]0x8FC7) Antigravity $([char]0x6253)$([char]0x5F00)",
	[switch]$SkipCompile
)
...
$SourcePath = Join-Path $WorkDir 'src\AntigravityExplorerCommand.cpp'
...
$DllPath = Join-Path $ExternalDir 'AntigravityExplorerCommand.dll'
$ObjPath = Join-Path $ExternalDir 'AntigravityExplorerCommand.obj'
$ManifestPath = Join-Path $ManifestDir 'AppxManifest.xml'
$AppxPath = Join-Path $PackageDir 'AntigravityContextMenu.appx'
$CertPath = Join-Path $PackageDir 'OpenWithAntigravityContextMenu.cer'
...
<Package
  ...
  <Properties>
    <DisplayName>Antigravity Context Menu</DisplayName>
  ...
  <Applications>
    <Application Id="AntigravityContextMenu"
      ...
      <Extensions>
        ...
        <com:Extension Category="windows.comServer">
          <com:ComServer>
            <com:SurrogateServer DisplayName="Antigravity Context Menu">
              <com:Class Id="$ClassId" Path="AntigravityExplorerCommand.dll" ThreadingModel="STA" />
            </com:SurrogateServer>
          </com:ComServer>
        </com:Extension>
      </Extensions>
...
function Set-ContextMenuSettings {
	...
	New-ItemProperty -Path $SettingsKey -Name 'ExePath' -Value $AntigravityExe -PropertyType ExpandString -Force | Out-Null
}

function Remove-LegacyRegistryVerbs {
	$keys = @(
		'HKCU:\Software\Classes\*\shell\Antigravity',
		'HKCU:\Software\Classes\Directory\shell\Antigravity',
		'HKCU:\Software\Classes\Directory\Background\shell\Antigravity',
		'HKLM:\Software\Classes\*\shell\Antigravity',
		'HKLM:\Software\Classes\Directory\shell\Antigravity',
...
function Ensure-SigningCertificate {
	...
	if (-not $cert) {
		$cert = New-SelfSignedCertificate `
			-Type Custom `
			-Subject $Publisher `
			-FriendlyName 'OpenWith Antigravity Context Menu' `
...
Assert-FileExists $AntigravityExe 'Antigravity.exe'
Compile-Dll
